...
- [ ] **DH7** — Final audit reports no unresolved blocking or high findings. *(Open. Twelve rounds have now run and each found something. Round 12 is recorded in `docs/audits/round-12-independent-audit.md`: eighteen findings, all accepted after verification — two blocking, four high, every one fixed except one stated as a limitation, and every code fix carrying a test that fails without it. The worst: every action form was refused in a real browser, and participant writes followed symlinks out of `participant/`. The criterion closes on a round that finds nothing, so it needs round 13 against the merge commit.)*
