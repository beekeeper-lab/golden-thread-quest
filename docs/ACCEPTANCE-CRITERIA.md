...
- [ ] **DH7** — Final audit reports no unresolved blocking or high findings. *(Open. Seven rounds have now run and each found something. Round 7 is recorded in `docs/audits/round-07-independent-audit.md`: twenty-six findings, one blocking and eight high, every one fixed and every code fix carrying a test that fails without it. Its theme is a check that cannot see what it is judging — two validators answered a question about one attempt's evidence using another's. The criterion closes on a round that finds nothing, so it needs round 8 against the merge commit.)*
