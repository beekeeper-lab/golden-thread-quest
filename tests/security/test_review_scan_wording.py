...
OVERSIZE_TEXT = "The scan found a file too large to read, so it could not be checked."


def _evidence_page_html(config: AppConfig) -> str:
    _review_page_html(config)
    page = config.generated_root / "evidence" / QUEST / "index.html"
    return page.read_text(encoding="utf-8")


def test_a_file_too_large_to_scan_gets_its_own_message(config: AppConfig) -> None:
    ...
    html = _review_page_html(config)
    assert OVERSIZE_TEXT in html
    assert SECRET_TEXT not in html, "an oversize file is not a secret finding"


def test_the_participants_evidence_page_does_not_call_a_link_a_secret(
    ...
    assert "Possible secret in your evidence" not in html
    assert "leads outside the evidence package" in html


def test_the_participants_evidence_page_still_reports_a_secret(config: AppConfig) -> None:
    package = _package(config)
    leaked = "ghp_abcdefghijklmnopqrstuvwxyz0123456789"  # secret-scan: allow
    (package / "leaked-notes.md").write_text(f"token={leaked}\n")

    assert "Possible secret in your evidence" in _evidence_page_html(config)
