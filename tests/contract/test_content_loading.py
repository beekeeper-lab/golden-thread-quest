...
from quest_app.content_loader import load_content, parse_acceptance_criteria, parse_sections
...
class TestBodyParsing:
    ...
    def test_unrecognised_heading_is_kept_under_a_slug(self) -> None:
        """An author's heading must never silently vanish from the page."""
        sections = parse_sections("## Mission\n\nx\n\n## Field Notes\n\ny\n")
        assert "field-notes" in sections
        assert sections["field-notes"][0] == "Field Notes"

    def test_criteria_ids_are_positional(self) -> None:
        criteria = parse_acceptance_criteria("1. First thing\n2. Second thing\n")
    ...
    def test_criterion_text_hash_changes_with_wording(self) -> None:
        ...
        first = parse_acceptance_criteria("1. The build succeeds\n")[0]
        second = parse_acceptance_criteria("1. The build succeeds quickly\n")[0]
...
class TestMarkdownSafety:
    ...
    def test_script_urls_are_dropped(self) -> None:
        rendered = render_markdown("[x](javascript:alert(1))")
        assert "javascript:" not in rendered or "<a" not in rendered

    def test_data_urls_are_dropped(self) -> None:
